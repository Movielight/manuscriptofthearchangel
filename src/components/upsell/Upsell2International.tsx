...
const Upsell2 = () => {
  ...
  return <div className="min-h-screen bg-gradient-to-b from-amber-50 via-white to-amber-50/30 flex flex-col">
      {/* Main Content */}
      <main className="flex-1 flex flex-col items-center justify-center px-4 py-12">
        ...
        duration: 0.6
          ...
          duration: 0.8
            ...
            <img src={angelicImage} alt="Divine Guidance" className="w-full max-w-md mx-auto rounded-2xl shadow-2xl shadow-amber-200/50" />
          </motion.div>
          ...
          duration: 0.5
            ...
            {showFallback && <div className="mt-6 space-y-4">
                <Button size="lg" className="w-full bg-amber-500 hover:bg-amber-600 text-white py-6 text-lg rounded-xl shadow-lg shadow-amber-200" onClick={() => window.location.href = '/manuscript'}>
                  Add Monthly Guidance
                  <ArrowRight className="ml-2 w-5 h-5" />
                </Button>
                <button className="w-full inline-flex items-center justify-center gap-2 text-slate-400 hover:text-slate-600 text-sm transition-colors" onClick={() => window.location.href = '/manuscript'}>
                  <X className="w-4 h-4" />
      ...
      <section className="px-4 py-12 bg-white/80">
        <div className="max-w-lg mx-auto">
          ...
          <div className="relative">
            ...
            <div className="overflow-hidden rounded-2xl bg-gradient-to-br from-amber-50 to-white border border-amber-100 p-6 min-h-[200px]">
              <AnimatePresence mode="wait" custom={direction}>
                ...
                duration: 0.3
                  ...
                  {/* Photo */}
                  <img src={testimonials[currentIndex].image} alt={testimonials[currentIndex].name} className="w-16 h-16 rounded-full object-cover border-2 border-amber-200 mb-4" />
                  
                  {/* Stars */}
                  <div className="flex gap-1 mb-3">
                    {[...Array(testimonials[currentIndex].rating)].map((_, i) => <Star key={i} className="w-4 h-4 fill-amber-400 text-amber-400" />)}
                  </div>
            ...
            {/* Navigation Buttons */}
            <button onClick={prevTestimonial} className="absolute left-0 top-1/2 -translate-y-1/2 -translate-x-3 w-8 h-8 bg-white rounded-full shadow-md flex items-center justify-center text-slate-600 hover:text-amber-600 transition-colors">
              <ChevronLeft className="w-5 h-5" />
            </button>
            <button onClick={nextTestimonial} className="absolute right-0 top-1/2 -translate-y-1/2 translate-x-3 w-8 h-8 bg-white rounded-full shadow-md flex items-center justify-center text-slate-600 hover:text-amber-600 transition-colors">
            ...
            }} className={`w-2 h-2 rounded-full transition-colors ${index === currentIndex ? 'bg-amber-500' : 'bg-slate-300'}`} />)}
            </div>
          </div>
